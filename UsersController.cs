...
namespace Service.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        [HttpPut]
        public ActionResult Put([FromBody]Model.UserInfo user)
        {
            try
            {
                var n = DAL.UserInfo.Instance.Update(user);
                if (n != 0)
            ...
            catch(Exception ex)
            {
                if (ex.Message.ToLower().Contains("null"))
                    return Json(Result.Err("密码、身份不能空"));
                else
        ...
        public ActionResult Post([FromBody]Model.UserInfo users)
        {
            ...
            catch(Exception ex)
            {
                if (ex.Message.ToLower().Contains("primary"))
                    ...
                    return Json(Result.Err("用户名、密码、身份不能空"));
                else
                    return Json(Result.Err(ex.Message));
            }
        }
        ...
        [HttpPost("check")]
        public ActionResult UserCheck([FromBody]Model.UserInfo user)
        {
            try
            {
                var result = DAL.UserInfo.Instance.GetModel(User.userName);
                if (result == null)
                ...
                else if (result.passWord == user.passWord)
                {
                    if (result.type == "管理员")
                    {
                        ...
                        return Json(Result.Err("只有管理员能进入后台管理"));
                }
                else
        ...
        [HttpPost("grnCheck")]
        public ActionResult grnUserCheck([FromBody]Model.UserInfo user)
        {
            try
            {
                var result = DAL.UserInfo.Instance.GetModel(user.userName);
